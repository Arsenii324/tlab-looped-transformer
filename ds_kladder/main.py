...
def main():
    ...
    # SUPERVISION-DENSITY LADDER: is supervise_k a continuous lever on useful depth?
    #
    # §4.14 compared exactly two densities, k=1 (terminal-only) and k=5 (dense), and found the
    # useful-depth plateau moves [8,16] -> [12,24] (mid 11.3 -> 17.0) at identical mu_rec=18. Two
    # points cannot distinguish a LEVER from a THRESHOLD: k=1 might be special (only the last loop
    # is ever supervised, so nothing anchors intermediate states), or depth might scale smoothly
    # with sparsity. Those imply different design advice, so it is worth five arms to separate.
    #
    # PREDICTION (written before running): if supervision density is a continuous lever, plateau
    # midpoint falls monotonically in k -- roughly 19.6 (k=1) > 17 (k=2) > 14 (k=3) > 11.3 (k=5)
    # ~ 11 (k=8). If instead k=1 is a threshold effect, k=2..8 cluster near 11.3 and only k=1 stands
    # apart. A NON-MONOTONE ordering means the effect is inside noise (§4.15: floor ~0.05 nats, and
    # plateau midpoints are quantised by the eval grid) and the two-point §4.14 result is luck.
    #
    # Grid includes 12 and 20 so this is directly comparable to §4.14's sweep, which the deep-terminal
    # job's grid was not (it lacks 12, forcing a restricted-grid comparison).
    TOK = 2_500_000
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    arms = []
    for k in (1, 2, 3, 5, 8):
        arms.append((f"kl_k{k}",
                     dataclasses.replace(center_model, state_renorm=False),
                     TrainConfig(run_name=f"kl_k{k}", batch_size=8, total_tokens=TOK,
