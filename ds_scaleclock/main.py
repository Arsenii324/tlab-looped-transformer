...
T0 = time.time()
MAX_SWEEP_SECONDS = 3.2 * 3600  # graceful stop; cancel-to-harvest still available earlier  # harvest ~17:30, before the 18:00 compute cutoff  # 5 arms x ~830s measured + eval overhead  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
class LoopedTransformer(nn.Module):
    def __init__(self, cfg: Config):
        ...
        # scale clock: +H params, zero-init => bit-identical to scale_clock=False at step 0
        ...
        for t in range(n_loops):
            ...
            with ctx:
                ...
                if self.clock_w is not None:
                    if log_rms0 is None:
                        log_rms0 = torch.log(h.float().pow(2).mean(-1, keepdim=True)
                                             .sqrt().clamp_min(1e-8)).detach()
                    _s = torch.log(h.float().pow(2).mean(-1, keepdim=True)
                                   .sqrt().clamp_min(1e-8)) - log_rms0
                    h_in = h_in * (1.0 + self.clock_w * _s.to(h_in.dtype))
...
class TrainConfig:
    ...
    supervise_k: int = 5
    supervise_k_final: int | None = None   # if set, k switches to this value partway through
    supervise_switch_frac: float = 0.5     # fraction of TOTAL STEPS at which the switch happens
...
def effective_k(step, total_steps, cfg):
    """Supervision density at this step. Constant unless `supervise_k_final` is set, in which case
    it switches ONCE at `supervise_switch_frac` of training -- a step function, not a ramp, because
    the k-ladder measured a THRESHOLD at k=1 rather than a continuous lever: intermediate k values
    (2,3,5) all behave alike, so a gradual ramp would spend most of training in the regime that
    provably does nothing and would confound 'when' with 'how much'."""
    if cfg.supervise_k_final is None:
        return cfg.supervise_k
    return cfg.supervise_k if step < cfg.supervise_switch_frac * total_steps else cfg.supervise_k_final
...
def run_arm(model_cfg: Config, train_cfg: TrainConfig, train_shard, val_shard, results):
    ...
    for step in range(total_steps):
        ...
        if (step % eval_every == 0 and step > 0) or step == total_steps - 1:
            ...
            if _WB is not None:
                try:
                    _WB.log({"step": step, "tokens": tokens_seen,
                             **{f"val_ce/r{r}": v for r, v in curve.items()},
                             "val_ce/best": min(curve.values()),
                             "val_ce/r1": curve[min(curve)]})
                except Exception as _e:
                    log(f"wandb log failed (non-fatal): {_e}")
...
def _wandb_start(name, mcfg, tcfg):
    """Redundant channel. `attach` works (verified) but needs a live local process; wandb does not.
    Wrapped so that no logging failure can kill a paid GPU job -- that trade is never worth it."""
    global _WB
    try:
        import wandb, dataclasses as _dc
        if not os.environ.get("WANDB_API_KEY"):
            log("no WANDB_API_KEY in env; skipping wandb"); return
        wandb.finish()
        _WB = wandb.init(project="tlab-loop-transformer", name=name, reinit=True,
                         config={**_dc.asdict(mcfg), **_dc.asdict(tcfg)},
                         settings=wandb.Settings(start_method="thread"))
        log(f"wandb up: {name}")
    except Exception as e:
        _WB = None
        log(f"wandb init failed (non-fatal): {e}")
...
def main():
    ...
    # THE DEEP FULL-BUDGET ARTIFACT: one arm, all the tokens that fit before the 18:00 harvest.
    #
    # Every headline in this report so far is a randomized U[4,32] model swept at INFERENCE to find
    # its best depth. That answers "how deep can this model usefully go", not "can a model that must
    # run many loops be good". This arm is the second thing.
    #
    # CONFIG CHOICE, and it deliberately differs from what was proposed to me. The proposal was fixed
    # L with CONSTANT terminal-only supervision. My own paired data at mu_rec=40 says that is the
    # dominated arm:
    #     dense      plateau [16,32]  mid 22.6  CE 5.4170
    #     terminal   plateau [32,48]  mid 39.2  CE 5.6051   (+0.188 vs dense)
    #     ANNEALED sw75 plateau [32,64] mid 45.3 CE 5.4466   (+0.030 vs dense)
    # Annealing reaches a DEEPER useful band than constant terminal-only (45.3 vs 39.2) at a sixth of
    # the CE cost. Launching constant terminal-only would knowingly pick the worse arm.
    #
    # Schedule U[32,48]: every training step uses at least 32 loops, so this is not a shallow model
    # swept deep -- it is trained deep throughout. Chosen over fixed L=40 because U[32,48] is the
    # configuration actually MEASURED above; a fixed-L-plus-annealing cell has never been run, and
    # scaling an unmeasured config to the full budget is the mistake this project has a rule against.
    #
    # PRE-REGISTERED, on the plateau (argmin is retired, §4.15), before launch:
    #   * plateau midpoint >= 32, i.e. this model's useful depth is genuinely in the tens of loops.
    #     Point prediction 45 (the 2.5M measurement), tolerated range 32-64.
    #   * absolute CE WORSE than the 90M headline (3.6146) by roughly +0.5 nats -- this arm gets ~25M
    #     tokens against 90M, and 0.398 nats/e-fold predicts +0.51. **A worse absolute CE is the
    #     EXPECTED result and is not a failure of the config**; recording that now so it cannot be
    #     read either way after the fact.
    #   * FALSIFIED if the plateau midpoint comes back near 22 (dense-like) -- that would mean the
    #     annealing effect does not survive a 10x token budget, which is exactly what the norm penalty
    #     did (-0.366 at 2.5M -> -0.030 at 90M) and is the live risk here.
    # ================================================================================
    # TWO DECISIONS THAT HAVE NEVER BEEN TESTED. Not a scale-up: the scale clock is already
    # running locally at 2.5M and its own pre-registration says CE is unresolvable at that
    # budget, so re-running the same arms with more tokens would buy nothing.
    #
    # (A) THE ANCHOR ACCOUNT'S OWN FALSIFIER (§4.18). The account says the useful band is set
    #     by how long the trajectory runs UNANCHORED at the end of training, and NOT by how
    #     densely it was anchored before that. Same switch point, different dense-phase
    #     density => the account predicts the SAME band.
    #       PREDICTED: sw90_k5 / sw90_k3 / sw90_k2 plateaus agree within the grid.
    #       FALSIFIED: bands order with k. Then density matters in its own right, the account
    #                  is wrong as stated, and §4.18 gets withdrawn rather than caveated.
    #     This is the cheapest way to kill a framing that currently post-dicts five results.
    #
    # (B) TOKEN-KEYED vs FRACTION-KEYED ANNEALING (§3.5 scale block, W8/R43). §3.5 recommends
    #     a FRACTION (sw90) while §4.12 puts loop-gain saturation at an absolute ~10-15M
    #     tokens. At 10M the two rules diverge sharply: sw90 switches at 9M (post-saturation),
    #     token-keyed at 2.25M (the absolute point the 2.5M evidence actually validated).
    #       PREDICTED (from the 4x erosion of the loop-1 margin, -0.035 -> -0.0092):
    #         tok225 keeps ΔCE@1 <= 0 while sw90 at 10M does not.
    #       This decides what §3.5 should recommend for a larger budget, which is the task's
    #       own "why does this work at scale" criterion. Currently UNTESTED and the report
    #       says so.
    # ================================================================================
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    base = dataclasses.replace(center_model, state_renorm=False)
    def T(nm, tok, **kw):
        d = dict(run_name=nm, batch_size=8, total_tokens=tok,
                 eval_every_tokens=max(1_000_000, tok // 10),
                 supervise_k=5, min_train_loops=4, max_train_loops=32, seed=0,
                 eval_loop_sweep=SWEEP)
        d.update(kw); return TrainConfig(**d)
    M25 = 2_500_000
    arms = [
        # (A) cheapest first, so an early cancel still answers the falsifier
        ("ak_sw90_k5", base, T("ak_sw90_k5", M25, supervise_k=5, supervise_k_final=1,
                               supervise_switch_frac=0.90)),
        ("ak_sw90_k3", base, T("ak_sw90_k3", M25, supervise_k=3, supervise_k_final=1,
                               supervise_switch_frac=0.90)),
        ("ak_sw90_k2", base, T("ak_sw90_k2", M25, supervise_k=2, supervise_k_final=1,
                               supervise_switch_frac=0.90)),
        ("ak_dense_k5", base, T("ak_dense_k5", M25)),          # in-job control for all three
        # (B) the expensive pair, last, so cancelling loses the least (DATASPHERE_NOTES rule)
        ("tk_frac90_10M", base, T("tk_frac90_10M", 10_000_000, supervise_k_final=1,
                                  supervise_switch_frac=0.90)),
        ("tk_tok225_10M", base, T("tk_tok225_10M", 10_000_000, supervise_k_final=1,
                                  supervise_switch_frac=0.225)),   # == 2.25M tokens absolute
    ]
    ...
    for name, r in results.items():
        ...
        w = r.get("clock_w_norm")
        if w is not None:
            print(f"  {name:<18} ||clock_w|| = {w:.5f}  (0 => model declined the clock)")
