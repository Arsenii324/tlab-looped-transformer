...
T0 = time.time()
MAX_SWEEP_SECONDS = 3.0 * 3600  # 5 arms x ~830s measured + eval overhead  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    # ANNEALING AT DEPTH -- combines the two strongest threads in the project.
    #
    # Thread 1 (deep-terminal, 3 points): terminal-only's useful-depth midpoint tracks mu_rec at
    # 1.09 / 1.00 / 0.98 for mu_rec = 18 / 32 / 40. At mu_rec=40 its useful band is loops 32-48 --
    # genuinely many useful loops -- but its CE is 5.6051 against the dense control's 5.4170.
    # Thread 2 (annealing, §4.17): switching to k=1 only for the FINAL phase keeps the depth shift
    # while cutting the CE price (constant terminal +0.21; annealed at 50% +0.041 avg over 2 seeds).
    #
    # The obvious combination has not been run: anneal at mu_rec=40. If the §4.17 mechanism carries
    # to a deep schedule, the result is a model whose useful band sits near 40 loops at close to the
    # dense arm's loss -- which is the task's literal objective (low perplexity BY exploiting many
    # loops) rather than a proxy for it.
    #
    # PRE-REGISTERED. sw90/sw75 at mu_rec=40 land at midpoint >= 30 (well above dense's 22.6) with CE
    # within ~0.05 of the in-job dense control. FALSIFIED if their midpoints come back near dense's
    # 22.6 (annealing does not carry to deep schedules -- the §4.17 effect would then be specific to
    # mu_rec=18) or if CE degrades toward the constant-terminal 5.6051 (the price returns at depth).
    #
    # mu_rec=40 via U[32,48]: the deepest schedule that FITS -- U[32,56] and U[40,72] both OOM'd on
    # this card at batch_size=8, and batch size is held fixed so these stay comparable to the arms above.
    # One seed per arm: 3 arms x ~30 min. The OOM guard from ds_deep2 is inherited.
    TOK = 2_500_000
    SWEEP = tuple(sorted({1,2,4,8,16,24,32,40,48,64,96,128}))
    def A(tag, kf, frac):
        return (f"da2_{tag}",
                dataclasses.replace(center_model, state_renorm=False),
                TrainConfig(run_name=f"da2_{tag}", batch_size=8, total_tokens=TOK,
                             eval_every_tokens=1_250_000, supervise_k=5,
                             supervise_k_final=kf, supervise_switch_frac=frac,
                             min_train_loops=32, max_train_loops=48, seed=1,
                             eval_loop_sweep=SWEEP))
    # SEED 1 of the mu_rec=40 annealing arms. Seed 0 gave sw90 a plateau of [24,48] (mid 33.9) at
    # CE 5.4394 -- a useful band spanning loops 24-48 -- but that is one seed, and §4.17's own
    # replication showed sw75's CE advantage FLIPPING SIGN between seeds while its depth held. So the
    # prediction here is split deliberately: the PLATEAU should reproduce (depth has been the robust
    # quantity in every replication so far, identical to the digit across 6 arms and 2 devices), and
    # the CE gap is the part that may not.
    # ONE ARM: constant terminal-only at mu_rec=40, SEED 1.
    #
    # The deliverable's method section rests on a single 2.5M paired comparison at mu_rec=40:
    #     annealed sw75   plateau [32,64]  mid 45.3  CE 5.4466
    #     constant term   plateau [32,48]  mid 39.2  CE 5.6051
    # i.e. annealing exceeds BOTH its endpoints on depth (a pure-interpolation prior says it should
    # land between them) at a sixth of the CE cost. That non-monotonicity is the surprising part and
    # the reason the method is worth stating -- and it is **n=1**.
    #
    # `tlab-deep-anneal2` already re-runs the ANNEALED arms and the dense control at seed 1. What it
    # lacks is the constant-terminal arm, without which the 45.3-vs-39.2 gap cannot be replicated.
    # This is that arm and nothing else.
    #
    # PRE-REGISTERED: constant terminal-only lands at midpoint ~39 (0.98*mu_rec, the ratio measured at
    # three schedules), BELOW the annealed arm's seed-1 midpoint. FALSIFIED if it comes back at or
    # above the annealed arm -- in which case the non-monotonicity was seed noise and §3.5's method
    # reduces to "use constant terminal-only", which is a materially different recommendation.
    arms = [("t1_mu40_term",
             dataclasses.replace(center_model, state_renorm=False),
             TrainConfig(run_name="t1_mu40_term", batch_size=8, total_tokens=2_500_000,
                          eval_every_tokens=1_250_000, supervise_k=1,
                          min_train_loops=32, max_train_loops=48, seed=1,
                          eval_loop_sweep=tuple(sorted({1,2,4,8,16,24,32,40,48,56,64,96,128}))))]
