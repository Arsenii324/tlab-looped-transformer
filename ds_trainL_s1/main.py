...
T0 = time.time()
MAX_SWEEP_SECONDS = 5.0 * 3600  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    train_shard, val_shard = pack_from_stream(stream_it, tok, n_train_tokens=60_000_000,
    ...
    # SEED-1 REPLICATION of the train-at-L sweep. The t/L collapse (§4.9) rests on five
    # single-seed arms, and its spread at t/L=2 (0.0225) is BELOW the ~0.06 single-arm noise
    # measured in §4.10 -- which is either a genuine structural collapse or luck. A second
    # seed distinguishes those, and nothing else does.
    ...
    TOKENS_PER_ARM = 10_000_000
    arms = []
    for L in (2, 4, 8, 16, 32):
        arms.append((f"trainL{L}_s1",
                     dataclasses.replace(center_model, state_renorm=False),
                     TrainConfig(run_name=f"trainL{L}_s1", batch_size=8,
                                 total_tokens=TOKENS_PER_ARM,
                                 eval_every_tokens=max(1_000_000, TOKENS_PER_ARM // 6),
                                 min_train_loops=L, max_train_loops=L, seed=1,
                                 eval_loop_sweep=tuple(sorted({1, 2, 4, 8, 16, 32, 48, 64, L})))))
