...
class LoopedTransformer(nn.Module):
    ...
    def readout(self, h, cos=None, sin=None):
    ...
    def forward(self, input_ids, n_loops, return_all_loops=True, h0_noise=0.0, supervise_idx=None):
        ...
        for t in range(n_loops):
            ...
            logits_per_loop.append(self.readout(h, cos, sin) if wanted else None)
...
def train_tokenizer():
    ...
    # SHIP THE VOCABULARY WITH THE RUN. Previously this kernel trained the BPE fresh and never saved
    # it, so the vocabulary that produced a checkpoint existed only as a side-effect of the run --
    # and a checkpoint shipped without its vocab cannot be evaluated by anyone else. The failure is
    # silent: a mismatched vocab reports CE ~ ln(4096) = 8.32 rather than raising. Identity with the
    # local tokenizer was verified after the fact (src/check_tokenizer_identity.py) and held, but
    # "it happened to match" is not a property to rely on twice.
    try:
        tok.save(os.path.join(OUT_DIR, "tokenizer.json"))
        log(f"saved tokenizer to {os.path.join(OUT_DIR, 'tokenizer.json')}")
    except Exception as e:
        log(f"WARNING: could not save tokenizer: {e}")
...
class TrainConfig:
    ...
    supervise_k: int = 5
    supervise_k_final: int = None      # annealing; None = constant density (mirrors src/train.py)
    supervise_switch_frac: float = 0.90
...
def main():
    ...
    train_shard, val_shard = pack_from_stream(stream_it, tok, n_train_tokens=92_000_000,
    ...
    # WHY THIS RUN EXISTS. §3.5 rests on the in-job (sw90 - dense) CE difference, measured at
    # TWO seeds: -0.0811 and -0.0609, mean -0.0710. A correction-propagation audit found the report
    # judging that number against the CUDA *dense* replicate floor (0.0150) where it looked
    # favourable and against the *terminal* floor (0.0541) where it did not -- and §4.15 never
    # established which applies to an ANNEALED arm, which is dense for 90% of training and terminal
    # for 10%. Against 0.0541 the headline is only 1.3x the floor.
    #
    # A same-config replicate would give "a floor". The PAIRED difference is the better object: each
    # seed yields one estimate of the quantity §3.5 actually claims, and n=4 gives an interval on it
    # rather than a floor to compare against. Seeds 0 and 1 exist (a3_*); this adds 2 and 3.
    #
    # PRE-REGISTERED, before the run: if the four paired differences straddle 0, or their mean is
    # within the 0.0541 terminal floor, §3.5's annealing recommendation is withdrawn to
    # "not resolved at this budget". If all four are negative and the mean holds near -0.07, the
    # claim stands on n=4 paired estimates and no longer needs a floor argument at all.
    SWEEP = (1, 2, 4, 8, 12, 16, 20, 24, 32, 48, 64)
    base = dataclasses.replace(center_model, state_renorm=False)
    def T(nm, seed, **kw):
        d = dict(run_name=nm, batch_size=8, total_tokens=2_500_000,
                 eval_every_tokens=312_500, supervise_k=5, min_train_loops=4,
                 max_train_loops=32, seed=seed, eval_loop_sweep=SWEEP)
        d.update(kw); return TrainConfig(**d)
    arms = []
    for sd in (2, 3):
        arms.append((f"kg_dense_s{sd}", base, T(f"kg_dense_s{sd}", sd)))
        arms.append((f"kg_sw90_s{sd}",  base, T(f"kg_sw90_s{sd}",  sd,
                     supervise_k_final=1, supervise_switch_frac=0.90)))
