...
T0 = time.time()
MAX_SWEEP_SECONDS = 3.0 * 3600  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    train_shard, val_shard = pack_from_stream(stream_it, tok, n_train_tokens=22_000_000,
    ...
    TOK = 10_000_000
    arms = [
        ("gate_control", dataclasses.replace(center_model, state_renorm=False),
         TrainConfig(run_name="gate_control", batch_size=8, total_tokens=TOK,
                      eval_every_tokens=2_000_000)),
        ("gate_convex", dataclasses.replace(center_model, state_renorm=False, convex_gate=True),
         TrainConfig(run_name="gate_convex", batch_size=8, total_tokens=TOK,
                      eval_every_tokens=2_000_000)),
    ]
