...
def main():
    ...
    # REPLICATION OF THE TWO BEST ANNEALING ARMS, control in-job, two seeds each.
    #
    # The switch-fraction series (one run per point, each compared to POOLED cross-job references)
    # produced three coherent monotone structures rather than one lucky arm:
    #     %@k=1   mid    gain     CE        vs dense mean
    #       0%    11.3   0.1114   5.3369       -
    #      10%    13.9   0.1495   5.2659     -0.0710   <- best CE in the project
    #      25%    17.0   0.1830   5.3061     -0.0308
    #      50%    17.0   0.2367   5.3711     +0.0342
    #     100%    17.0   0.2647   5.5512     +0.2143
    # Depth saturates at 25%, gain rises monotonically, CE is U-shaped with its minimum at 10%.
    #
    # What is NOT yet established: every point is a single run, and every dense number it was compared
    # against came from a different job (cross-job dense spread 0.0334, vs an sw90 margin of 0.0583 --
    # close enough that an in-job control is required before the claim is made). Two seeds each, and
    # the dense control runs in THIS job on THIS shard and tokenizer.
    #
    # PRE-REGISTERED. sw90 beats its in-job dense control on CE at BOTH seeds, by roughly 0.03-0.07,
    # AND has a midpoint >= 12.6 (deeper than dense's 11.3). sw75 matches dense on CE within ~0.03
    # with midpoint 17.0. FALSIFIED if sw90's CE advantage vanishes against the in-job control, or if
    # its sign flips at either seed -- in which case the U-shape was cross-job noise and the honest
    # result reverts to "annealing buys depth at a small CE cost" (the sw50 finding), not "for free".
    TOK = 2_500_000
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    def A(tag, kf, frac, seed):
        return (f"a3_{tag}_s{seed}",
                dataclasses.replace(center_model, state_renorm=False),
                TrainConfig(run_name=f"a3_{tag}_s{seed}", batch_size=8, total_tokens=TOK,
    ...
    arms = []
    for seed in (0, 1):
        arms.append(A("sw90",  1,    0.90, seed))
        arms.append(A("sw75",  1,    0.75, seed))
        arms.append(A("dense", None, 0.50, seed))   # kf=None -> constant k=5, the in-job control
