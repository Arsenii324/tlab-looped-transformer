...
def main():
    ...
    # SUPERVISION ANNEALING -- MY OWN PROPOSAL (not the author's graded idea), motivated by two
    # measurements from this project rather than by analogy:
    #
    #   (a) §4.14 + the CUDA replication: terminal-only (k=1) reliably moves the useful-depth plateau
    #       up ~1.7x, but charges an unpredictable amount of CE -- inside the noise floor on MPS,
    #       +0.191 nats on CUDA, where 16 loops of the terminal model lose to ONE loop of the dense
    #       control. The depth mechanism works; its price is uncontrolled. That is the open problem.
    #   (b) The k-ladder: k=2,3 behave like k=5, not like k=1 (plateau mid 12.6/12.6 vs 17.0). So the
    #       effect is a THRESHOLD at "no intermediate supervision at all", not a dial. You therefore
    #       cannot buy a partial dose by choosing an intermediate k.
    #
    # If you cannot take a partial dose in DENSITY, the remaining axis is TIME: spend most of training
    # dense (which is where the CE comes from) and only the final phase terminal-only (which is where
    # the depth reorganisation would have to happen). Two outcomes, both informative:
    #   - if late-switch arms reach terminal-like depth (mid ~17) at dense-like CE, the price is
    #     controllable and this is a usable mechanism for the task's actual objective;
    #   - if depth tracks how LONG the model spent at k=1, the reorganisation is slow and there is no
    #     cheap version -- which closes the direction honestly.
    # `reverse` (k=1 first, then dense) is the control that distinguishes "the last phase decides" from
    # "any exposure to k=1 decides"; without it a positive result has two readings.
    TOK = 2_500_000
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    def A(name, k0, k1, frac):
        return (name, dataclasses.replace(center_model, state_renorm=False),
                TrainConfig(run_name=name, batch_size=8, total_tokens=TOK,
                             eval_every_tokens=1_250_000, supervise_k=k0,
                             supervise_k_final=k1, supervise_switch_frac=frac,
                             min_train_loops=4, max_train_loops=32, seed=0,
                             eval_loop_sweep=SWEEP))
    arms = [A("an_sw50", 5, 1, 0.50),    # half dense, half terminal
            A("an_sw75", 5, 1, 0.75),    # terminal only for the last quarter
            A("an_sw90", 5, 1, 0.90),    # terminal only for the last tenth
            A("an_rev50", 1, 5, 0.50)]   # control: terminal FIRST, then dense
