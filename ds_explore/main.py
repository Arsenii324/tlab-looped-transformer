...
T0 = time.time()
MAX_SWEEP_SECONDS = 4.5 * 3600  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    train_shard, val_shard = pack_from_stream(stream_it, tok, n_train_tokens=14_000_000,
    ...
    TOK = 10_000_000
    # EXPLORATION-NOISE SWEEP. The task names "exploration во время лупов" and cites EBT (whose
    # loop is Langevin descent); nothing here had tested it. Motivated by §4.3's own measurement:
    # consecutive increments align at cos -> 0.9999, i.e. the trajectory is maximally coherent and
    # never deviates. sigma = 0.0 is bit-exactly the existing model, so the control is in the sweep.
    TOK = 6_000_000
    arms = []
    for sg in (0.0, 0.05, 0.15, 0.4):
        arms.append((f"expl_{sg}",
                     dataclasses.replace(center_model, state_renorm=False, explore_noise=sg),
                     TrainConfig(run_name=f"expl_{sg}", batch_size=8, total_tokens=TOK,
