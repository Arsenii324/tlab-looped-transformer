...
T0 = time.time()
MAX_SWEEP_SECONDS = 15.0 * 3600  # harvested at 18:00; early-stop-safe  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    # TRAIN-AT-L sweep: separate models trained at FIXED loop count L, each evaluated at its own L.
    # This is a different curve from everything else in this project, which is EVAL-AT-T (train once
    # at random 4-32, sweep inference depth). The literature reports both interchangeably and they
    # disagree: eval-at-T peaks near mu_rec and degrades (SCSE 50M: 123.1 -> 156.4 over T=8..48; this
    # project: optimum 8, loop-1 parity at 105), while train-at-L is reported monotone in L
    # (LoopFormer at ~1B on The Pile). The task's "количество лупов - чем больше, тем лучше" reads as
    # train-at-L, and this project had exactly ONE train-at-L point before this sweep.
    #
    # Token-budgeted, NOT wall-clock: cost per step scales with L, so a wall-clock budget would hand
    # 16x more data to the L=2 arm than the L=32 arm and manufacture exactly the monotonicity we are
    # testing for. Every arm gets TOKENS_PER_ARM.
    # FULL-BUDGET RUN AT A FIXED LOOP COUNT.
    # FIXED_L comes from the train-at-L sweep's argmin, passed by env at submit time so nothing in
    # this file needs editing at the decision point. This is the config that answers the task
    # directly if train-at-L is monotone: best perplexity AND many loops in one number, instead of
    # the randomized 4-32 schedule the current headline uses -- which §4.11 shows places the optimum
    # at roughly half of mu_rec.
    # Early-stop-safe by construction: results.json AND a checkpoint are written at every eval
    # boundary, so this can be launched without fitting the deadline and harvested at 18:00.
    FIXED_L = int(os.environ.get("FIXED_L", "16"))
    TOK = int(os.environ.get("TOK", "90000000"))
    log(f"FIXED-L RUN: L={FIXED_L}, target tokens={TOK:,}")
    arms = [(f"fixedL{FIXED_L}",
             dataclasses.replace(center_model, state_renorm=False),
             TrainConfig(run_name=f"fixedL{FIXED_L}", batch_size=8, total_tokens=TOK,
                          eval_every_tokens=4_000_000,
                          min_train_loops=FIXED_L, max_train_loops=FIXED_L,
                          eval_loop_sweep=tuple(sorted({1,2,4,8,16,24,32,48,64,96,128,FIXED_L}))))]
    ...
    log("\n=== TRAIN-AT-L RESULT (each arm at its OWN trained L) ===")
    for nm, r in results.items():
        Lv = r["train_cfg"]["min_train_loops"]
        h = r.get("history") or []
        if h:
            c = h[-1]["val_curve"]
            own = c.get(str(Lv), c.get(Lv))
            best = min(c, key=c.get)
            log(f"  L={Lv:>2}  tokens={h[-1]['tokens']:>10,}  CE@own_L={own}  "
                f"best_r={best} CE={c[best]:.4f}  CE@1={c.get('1', c.get(1))}")
