...
T0 = time.time()
MAX_SWEEP_SECONDS = 6.8 * 3600  # harvest ~17:30, before the 18:00 compute cutoff  # 5 arms x ~830s measured + eval overhead  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    TOK = 40_000_000          # ceiling; the wall-clock guard below is what actually binds
    SWEEP = tuple(sorted({1,2,4,8,16,24,32,40,48,56,64,80,96,128}))
    arms = [("df_mu40_sw75",
             dataclasses.replace(center_model, state_renorm=False),
             TrainConfig(run_name="df_mu40_sw75", batch_size=8, total_tokens=TOK,
                          eval_every_tokens=5_000_000, supervise_k=5,
                          supervise_k_final=1, supervise_switch_frac=0.75,
                          min_train_loops=32, max_train_loops=48, seed=0,
