...
def main():
    ...
    TOK = 10_000_000
    # FIXED-GATE SWEEP, not an A/B (METHODS.md rule 2). g is a swept constant, and g=1.0 is
    # EXACTLY the ungated model (verified bit-identical), so the control is inside the sweep rather
    # than beside it. The readout is the ORDERING of best-CE and optimum-location across g, which
    # survives noise that a single pairwise difference would not. Zero extra parameters at every g.
    TOK = 6_000_000
    arms = []
    for g in (0.25, 0.5, 0.75, 1.0):
        arms.append((f"gsweep_{g}",
                     dataclasses.replace(center_model, state_renorm=False,
                                          convex_gate=True, fixed_gate=g),
                     TrainConfig(run_name=f"gsweep_{g}", batch_size=8, total_tokens=TOK,
