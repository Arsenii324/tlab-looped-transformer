...
T0 = time.time()
MAX_SWEEP_SECONDS = 4.0 * 3600  # 5 arms x ~830s measured + eval overhead  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    # DOES THE ANNEALING ADVANTAGE SURVIVE 4x THE TOKEN BUDGET?
    #
    # §4.17 is confirmed at 2.5M tokens: sw90 (terminal-only for the last 10% of steps) beat its
    # in-job dense control at both seeds by -0.0811 / -0.0609, with a wider useful-depth band.
    # The task explicitly asks for a design that "plausibly keeps working at larger scale", and this
    # project has one hard lesson about screening budgets: the norm penalty gives -0.366 nats at 2.5M
    # tokens and only -0.030 at 90M -- a 12x shrinkage. An effect measured at 2.5M is NOT evidence
    # about the budget that matters.
    #
    # So: the same comparison at 10M tokens, 4x the budget, everything else identical. In-job dense
    # control, one seed (two would double an already-2.5h job; the 2.5M result already has two seeds,
    # and what is being tested here is the BUDGET axis, not seed noise).
    #
    # PRE-REGISTERED. If sw90's CE advantage holds at roughly -0.03 to -0.08 with a plateau midpoint
    # above dense's, the mechanism is budget-robust and §4.17 can be stated as a design recommendation.
    # If it shrinks toward zero the way the norm penalty did, then §4.17 is a small-budget phenomenon
    # and must be labelled as such -- which would be the single most important caveat in this report,
    # because §8 currently puts annealing forward as the answer.
    # If it REVERSES, §4.17 is withdrawn as a recommendation and kept only as a measurement.
    TOK = 10_000_000
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    def A(tag, kf, frac):
        return (f"as_{tag}",
                dataclasses.replace(center_model, state_renorm=False),
                TrainConfig(run_name=f"as_{tag}", batch_size=8, total_tokens=TOK,
                             eval_every_tokens=2_500_000, supervise_k=5,
                             supervise_k_final=kf, supervise_switch_frac=frac,
                             min_train_loops=4, max_train_loops=32, seed=0,
                             eval_loop_sweep=SWEEP))
    arms = [A("10M_sw90",  1,    0.90),
            A("10M_dense", None, 0.50)]
