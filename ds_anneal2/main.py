...
def main():
    ...
    # ANNEALING REPLICATION, with the control IN THE SAME JOB.
    #
    # `an_sw50` (dense for the first half of steps, terminal-only for the second) reached terminal-only's
    # full depth -- plateau midpoint 17.0, identical to constant k=1 -- with CE 5.3711 against a pooled
    # dense reference of 5.3369 (5 runs) and a terminal reference of 5.5512 (2 runs). That is 84% of
    # terminal-only's CE penalty recovered while keeping its depth. Two weaknesses, both fixed here:
    #   (1) every dense number it was compared against came from a DIFFERENT job, and the measured
    #       cross-job dense spread is 0.0074-0.0334 -- the same order as the effect being claimed.
    #       Here the control runs in the same job, same data shard, same tokenizer.
    #   (2) n=1. Two seeds per condition.
    #
    # PREDICTION (pre-registered): an50 midpoint 17.0 at both seeds (depth is the robust part -- it has
    # reproduced identically across 6 arms and 2 devices), and an50 CE above dense by roughly 0.02-0.05
    # but far below the ~0.21 that constant terminal-only costs. FALSIFIED if an50's midpoint comes back
    # at ~11.3 (dense-like) -- that would mean the 17.0 was a one-run fluke and the whole annealing lead
    # closes.
    TOK = 2_500_000
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    arms = []
    for seed in (0, 1):
        arms.append((f"a2_an50_s{seed}",
                     dataclasses.replace(center_model, state_renorm=False),
                     TrainConfig(run_name=f"a2_an50_s{seed}", batch_size=8, total_tokens=TOK,
                                  eval_every_tokens=1_250_000, supervise_k=5,
                                  supervise_k_final=1, supervise_switch_frac=0.5,
                                  min_train_loops=4, max_train_loops=32, seed=seed,
                                  eval_loop_sweep=SWEEP)))
        arms.append((f"a2_dense_s{seed}",
                     dataclasses.replace(center_model, state_renorm=False),
                     TrainConfig(run_name=f"a2_dense_s{seed}", batch_size=8, total_tokens=TOK,
                                  eval_every_tokens=1_250_000, supervise_k=5,
                                  min_train_loops=4, max_train_loops=32, seed=seed,
                                  eval_loop_sweep=SWEEP)))
