"""DataSphere driver: duo-causal attention + scale-invariant depth gate, seed 2.
...
def main():
    ...
    train_shard, val_shard = pack_from_stream(stream_it, tok, n_train_tokens=22_000_000, n_val_tokens=3_000_000)

    SWEEP = tuple(sorted({1, 2, 4, 8, 12, 16, 20, 24, 32, 48, 64}))
    M25 = 10_000_000

    def make_tcfg(name):
        return TrainConfig(run_name=name, batch_size=8, total_tokens=M25, eval_every_tokens=1_000_000,
                           supervise_k=5, min_train_loops=4, max_train_loops=32, seed=2, eval_loop_sweep=SWEEP)

    arms = [
        ("rec_dense_s2", Config(state_renorm=False), make_tcfg("rec_dense_s2")),
        ("rec_sw90_s2",  Config(state_renorm=False),
         dataclasses.replace(make_tcfg("rec_sw90_s2"), supervise_k_final=1, supervise_switch_frac=0.90)),
    ]
    ...
    log("=== RECOMMENDED-METHOD ARTIFACT RUN COMPLETED ===")
