"""DataSphere driver: duo-causal attention + scale-invariant depth gate, seed 1.
...
def main():
    ...
    def make_tcfg(name):
        return TrainConfig(run_name=name, batch_size=8, total_tokens=M25, eval_every_tokens=500_000,
                           supervise_k=5, min_train_loops=4, max_train_loops=32, seed=1, eval_loop_sweep=SWEEP)

    arms = [
        ("dc_control_s1", Config(state_renorm=False), make_tcfg("dc_control_s1")),
        ("dc_w2_s1",      Config(state_renorm=False, kv_window=2), make_tcfg("dc_w2_s1")),
        ("dc_w3_s1",      Config(state_renorm=False, kv_window=3), make_tcfg("dc_w3_s1")),
        ("dg_norm_s1",    Config(state_renorm=False, depth_gate_mode="state_norm"), make_tcfg("dg_norm_s1")),
    ]
