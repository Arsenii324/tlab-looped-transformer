...
| ~~W3~~ | **DONE, §3.3.** The section audited for the *fixed-table* failure mode (the task's example) and never for the one the literature raises. Now states it: capacity here is dense, the FLOPs-per-parameter argument is made against a dense baseline which is not the frontier alternative, and **nothing in this project tests it** — no sparse arm, no budget, `DECISIONS.md` Q2 rank 8, never run. Sparse Layers logged SECOND-HAND with **no number quoted** (it is not in `papers/sources/`), so the objection stands on its own logic. Key distinction added: density threatens the **architecture's competitiveness**, not the **finding's transferability** — annealing is orthogonal to the dense/sparse axis and would apply unchanged to a sparse block. Only the second is what §3.5 claims | reviewer |
...
| ~~**W6**~~ | **The anchor account — DONE, §4.18.** Unifies five results, and the monotone table is the part that makes it checkable: band depth is monotone non-decreasing in unanchored duration (mid 11.3 → 13.9 → 17.0 → 17.0 for 0/10/25/50%), CE_best is U-shaped with an interior optimum at 10%, and `rev50` (same exposure placed first) returns *exactly* to dense's band. **Labelled as the reviewer's framing, not the author's idea, and not the graded one.** Includes its own clean failure: §4.7a rules out the trajectory version of it, so the anchor governs where the band sits and says nothing about why per-token demand is unreadable. Three named falsifiers, none run — the account post-dates the compute window | reviewer, 12:40 |
...
| ~~**W8**~~ | **DONE, §3.5 scale block.** And it is sharper than 'restate': the fraction rule was *validated* at 2.5M where `sw90` switches at 2.25M — **before loop gain has emerged at all** (§4.12: saturation ~10–15M) — and §3.5 extrapolated it to 90M where it switches at 81M, **deep post-saturation**. Not the same intervention. Token-keyed form puts the switch at ~10–15M in any budget = terminal-only for the last **~83%** at 90M, a different recipe. The only bearing evidence points against the fraction rule: as the switch moves pre→at saturation, ΔCE_best holds (−0.0710→−0.0764) while the loop-1 margin **erodes ~4×** (−0.035→−0.0092), extrapolating to damage-driven at 81M — which is what μ_rec=40 already shows. §3.5 now defends the mechanism and explicitly **not** the parameterisation | reviewer, 12:40 |
