...
| S3 | `refs/tags/main-backup-20260823` carries 4 blobs >100MB (1.83 GB); every branch is clean | **HIGH (push)** | **VERIFIED** — my first check said no tag carried them and was buggy (missing `%(rest)`); the review was right. `git push <branch>` is fine; **`--tags` / `--mirror` will be rejected**. Tag left intact per the no-deletion rule; recorded in `OPS.md` |
...
| S6 | Kaggle checkpoints carry incomplete `model_cfg` (15–18 of 23 fields); `readout_mode` would silently mis-evaluate if the default changed | MEDIUM | **OPEN** — correct today (verified: kernel `readout()` reduces to `readout_mode="norm"`, test [7] pins kernel-vs-src at 0.0). Nothing asserts the correspondence |
| S7 | `train.py` persisted only `state_norms[0]` and `[-1]` while `forward()` computes the full list — a direct violation of `CLAUDE.md:40`, and `results.json` DID survive every DS job, so per-loop diagnostics would have survived the ~20 lost checkpoints | MEDIUM | **FIXED** — full per-loop `state_norms`, `n_loops`, `supervise_k_eff` and per-loop train CE now persisted every eval |
| S8 | `evaluate()` draws from the training RNG, so eval cadence perturbs training data order — two runs differing in `eval_every_tokens` are NOT paired even at the same seed | MEDIUM | **OPEN, disclosed.** Benign for the headline (control and penalty share seed, cadence, step 43,944 and token count), so D1's paired claim holds. It is exactly the trap the local annealing run hit from the other side |
...
| S11 | Stale `baseline_nonlooped` (296,960 tokens) sits where it could be mistaken for §4.4's result; `SKIP_DOCS_FOR_TOKENIZER` is dead code; local vs Kaggle val sets are different document ranges | LOW | **OPEN.** The third is substantive: it is a concrete mechanism for the ~0.04-nat local-vs-kernel offset the report attributes to "a different validation batch draw" — both readings are live and the report should say so |
