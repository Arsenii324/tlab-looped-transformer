...
T0 = time.time()
MAX_SWEEP_SECONDS = 3.0 * 3600  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    # THIRD SCALING POINT, take 2: mu_rec=40 via U[32,48]. BISECTION on the memory ceiling.
    #   max_loops=40 (mu=32 pair)  -> fits
    #   max_loops=56 (mu=44 pair)  -> OOM, both arms, on the first forward
    #   max_loops=48 (this)        -> the midpoint; 94s tells us either way, thanks to the guard
    # Everything else is held at the mu=18/32 values (batch_size=8 above all, since shrinking it to
    # buy memory would change the gradient noise scale and break the scaling series it belongs to).
    # ORIGINAL RATIONALE (mu=44 attempt), unchanged and still what this tests:
    #
    # The two complete pairs already show what was pre-registered -- useful depth scales with trained
    # depth (dense mid 11.3 -> 22.6, terminal 19.6 -> 32.0 as mu_rec goes 18 -> 32) -- so this arm is
    # NOT needed for that claim. It is here for the finding that came out of those pairs and was not
    # predicted: the terminal-only CE penalty SHRINKS as the schedule deepens, +0.1913 at mu=18 to
    # +0.0702 at mu=32. If it keeps falling at mu=44, the price of the one shape-changing intervention
    # is controllable by training depth, which is the most task-relevant lead this project has. If it
    # flattens or reverses, the mu=18 -> 32 drop was two points and a line through them.
    #
    # mu_rec=44 via U[32,56], NOT the lost U[40,72]: 72 loops OOM'd and 40 fit, so 56 is the largest
    # max-loop count with a reasonable chance of fitting. Batch size is held at 8 -- shrinking it to
    # buy memory would change the gradient noise scale and break comparability with the arms above,
    # which is the whole point of the series.
    TOK = 2_500_000
    arms = []
    for k, tag in ((1, "term"), (5, "dense")):
        arms.append((f"d3_mu40_{tag}",
                     dataclasses.replace(center_model, state_renorm=False),
                     TrainConfig(run_name=f"d3_mu40_{tag}", batch_size=8, total_tokens=TOK,
                                  eval_every_tokens=1_250_000, supervise_k=k,
                                  min_train_loops=32, max_train_loops=48,
                                  eval_loop_sweep=tuple(sorted({1,2,4,8,16,24,32,48,64,96,128})))))

    for name, mcfg, tcfg in arms:
        if time.time() - T0 > MAX_SWEEP_SECONDS:
            log(f"SWEEP TIME BUDGET HIT before arm {name}, stopping sweep here")
            break
        # OOM in one arm must not kill the job. The mu=56 run (bt18ei0ieejnek71cmtd) died on
        # dt_mu56_term's FIRST forward and never reached dt_mu56_dense -- so a memory failure in one
        # arm cost the paired control, which would have fit. A deep arm not fitting is information.
        try:
            results = run_arm(mcfg, tcfg, train_shard, val_shard, results)
        except torch.cuda.OutOfMemoryError as e:
            log(f"=== ARM {name} OOM -- SKIPPED, continuing sweep: {str(e)[:200]}")
            results[name] = {"oom": True, "error": str(e)[:400], "history": []}
            torch.cuda.empty_cache()
        # (No extra persist needed here: run_arm already writes RESULTS_PATH at every eval.)
