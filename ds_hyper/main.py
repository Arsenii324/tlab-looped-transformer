...
T0 = time.time()
MAX_SWEEP_SECONDS = 2.5 * 3600  # 5 arms x ~830s measured + eval overhead  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    # THE TWO AXES OF THE STANDARD RECIPE THIS PROJECT NEVER SCREENED.
    #
    # Every result here uses lr=3e-3 and weight_decay=0.05, and neither was ever tested:
    #   * lr=3e-3 was tuned under the CENTER config, which had state_renorm=True. Turning that off
    #     changed ||h|| by three orders of magnitude and the gradient regime with it. The LR has
    #     never been re-swept in the regime actually being run. It is also ~10x what Sharma & Vu use
    #     at this parameter scale (3e-4) and 6x Huginn's 5e-4.
    #   * weight_decay=0.05 is not one of the five ablation axes and was never varied at all. The
    #     field's converged pretraining value is 0.1 (SCSE at 22M, Loopie at 20B); Sharma & Vu use 0.01.
    #
    # This does NOT change the headline config -- an unscreened axis is a hole in the report, a
    # screened axis with a null result strengthens it, and a CHANGED axis at hour 12 would invalidate
    # comparability across every arm already run. The purpose is to be able to say what the cost of
    # the inherited value is, not to adopt a new one.
    #
    # Reference arm (lr 3e-3, wd 0.05) is included so both series are measured IN-JOB against the
    # configuration every other result in the report used -- §4.15 says cross-job drift is
    # 0.0074-0.0334 and larger at deep schedules, which is the same order as any effect expected here.
    #
    # PRE-REGISTERED: if lr=3e-3 is within ~0.05 of the best of {1e-3, 6e-3}, the inherited value is
    # defensible and §6.0b can say so with a number. If 1e-3 wins by more than that, every result in
    # this report was measured at a stated handicap -- which is reportable, not fatal, and is exactly
    # the kind of thing §6.0b exists to surface.
    TOK = 2_500_000
    SWEEP = tuple(sorted({1,2,4,8,12,16,20,24,32,48,64}))
    def A(tag, lr, wd):
        return (f"hp_{tag}",
                dataclasses.replace(center_model, state_renorm=False),
                TrainConfig(run_name=f"hp_{tag}", batch_size=8, total_tokens=TOK,
                             eval_every_tokens=1_250_000, supervise_k=5,
                             lr=lr, min_lr=lr/10.0, weight_decay=wd,
                             min_train_loops=4, max_train_loops=32, seed=0,
                             eval_loop_sweep=SWEEP))
    arms = [A("ref_lr3e3_wd05", 3e-3, 0.05),   # the configuration every other result used
            A("lr1e-3",         1e-3, 0.05),
            A("lr6e-3",         6e-3, 0.05),
            A("wd0",            3e-3, 0.0),
            A("wd0.01",         3e-3, 0.01),
