...
def main():
    ...
    # THIRD SCALING POINT (mu_rec=44), after the mu=56 pair was lost to CUDA OOM.
    #
    ...
    for k, tag in ((1, "term"), (5, "dense")):
        arms.append((f"d2_mu44_{tag}",
                     dataclasses.replace(center_model, state_renorm=False),
                     TrainConfig(run_name=f"d2_mu44_{tag}", batch_size=8, total_tokens=TOK,
                                  eval_every_tokens=1_250_000, supervise_k=k,
                                  min_train_loops=32, max_train_loops=56,
