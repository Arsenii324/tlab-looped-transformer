...
def main():
    ...
    def make_tcfg(name):
        return TrainConfig(run_name=name, batch_size=4, total_tokens=M25, eval_every_tokens=500_000,
    ...
    # The 896 arm alone, at half the batch: it OOM'd in tlab-width-s0 sharing a T4 with two other
    # arms. 32.6M params under full BPTT over up to 32 loops is the memory-heaviest thing this
    # project has run. FOLLOW-UP: closes the 12x point that sec4.31 settles only at initialisation.
    arms = [
        ("w896_s0", Config(state_renorm=False, hidden_size=896, n_heads=8, n_kv_heads=4,
                           head_dim=112, intermediate_size=2688), make_tcfg("w896_s0")),
    ]
