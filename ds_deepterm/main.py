...
T0 = time.time()
MAX_SWEEP_SECONDS = 9.5 * 3600  # 6 arms, deep schedules; harvested at 18:00 regardless  # Kaggle's hard GPU-session ceiling is 12h; this stops gracefully
...
def main():
    ...
    # DEEP TERMINAL-ONLY: does the 0.89*mu_rec rule hold at larger mu_rec?
    #
    # §4.14 found that terminal-only supervision (supervise_k=1) puts the optimum at 0.89*mu_rec
    # instead of the 0.44 that dense supervision gives -- the only intervention measured in this
    # project that changes the SHAPE of the depth curve rather than the model's position on it.
    # If that ratio is a property of the supervision scheme rather than of mu_rec=18 specifically,
    # then training terminal-only at a deeper schedule should put the optimum proportionally deeper:
    #
    #     mu_rec=18 -> optimum ~16 (measured)
    #     mu_rec=32 -> optimum ~28   PREDICTED
    #     mu_rec=56 -> optimum ~50   PREDICTED
    #
    # An optimum near 50 would be this project's first demonstration of genuinely many useful loops,
    # which is the task's central ask. If instead the optimum saturates around 16-20 regardless of
    # mu_rec, then §4.14's effect is a one-off shift and not a lever, which is equally worth knowing.
    # Each arm carries its own dense (k=5) control at the same mu_rec so the ratio is measured, not
    # inferred from §4.14's single schedule.
    TOK = 2_500_000  # = §4.14's budget: proven sufficient to locate an optimum, and makes mu18 a replication
    arms = []
    for lo, hi in ((4, 32), (24, 40), (40, 72)):
        mu = (lo + hi) // 2
        for k, tag in ((1, "term"), (5, "dense")):
            arms.append((f"dt_mu{mu}_{tag}",
                         dataclasses.replace(center_model, state_renorm=False),
                         TrainConfig(run_name=f"dt_mu{mu}_{tag}", batch_size=8, total_tokens=TOK,
                                      eval_every_tokens=1_250_000, supervise_k=k,
                                      min_train_loops=lo, max_train_loops=hi,
